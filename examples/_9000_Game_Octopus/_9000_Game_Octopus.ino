...
// if you want to run this game on the ESPresso Lite V1.0, change boardType to ESPESPERT_BOARD_ESPRESSO_LITE1
byte boardType = ESPERT_BOARD_ESPRESSO_LITE2;
ESPert espert;
...
const uint8_t backgroundBitmap[] PROGMEM = { // background.png
  ...
  0xB8, 0xDE, 0xC9, 0xDE, 0xF3, 0xF6, 0x9C, 0xFF, 0xE7, 0xAB,
  0xBF, 0xFF, 0xE0, 0xE5, 0xFD, 0xF9, 0xFD, 0xFF, 0xFF, 0xFF,
  0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF,
  0xFF, 0xFF, 0xFF, 0xFF
};
...
byte button1Pin = 0;
byte button2Pin = (boardType == ESPERT_BOARD_ESPRESSO_LITE1) ? 2 : 13;
ESPert_Button button1;
...
bool checkShowHighScoreTime() {
  ...
  if (showHighScoreTime > 0 && (buttonPressed == 1 || buttonPressed == 2)) {
    if (++showHighScoreTime > 3) {
      showHighScoreTime = 0;
      isGameTypeVisibled = false;
    } else {
      showHighScore(buttonPressed);
      finished = false;
    }
  }
...
void showResetScreen(byte gameTypeValue) {
  ...
  if (gameTypeValue == 1 || gameTypeValue == 2) {
    ...
    hourDigitImage[0] = numberBitmap[1];
    hourDigitImage[1] = numberBitmap[2];

    for (int i = 0; i < 2; i++) {
      isHourDigitVisibled[i] = true;
      isMinuteDigitVisibled[i] = true;
  ...
  for (int i = 0; i < 6; i++) {
    isDiverStepVisibled[i] = true;
    diverStepImage[i] = (i == 0) ? onTheBoatBitmap[1] : diverBitmap[((i - 1) * 2) + 1];

    for (int j = 0; j < 5; j++) {
      if (i < maxTentacleCount[j]) {
        isTentacleVisibled[j][i] = true;
      }
    }

    if (i < 2) {
      isRemainderVisibled[i] = true;
...
void checkButtons() {
  bool isPressed = (digitalRead(button1Pin) == LOW) ? true : false;

  if (isPressed != isButton1Pressed) {
    isButton1Pressed = isPressed;

    if (isButton1Pressed) {
      if (buttonPressed == 0 && (add3Score == 0 || !isPlaying)) {
        buttonPressed = 1;

        if (!isPlaying || (isPlaying && (miss == 4 || caught == 0))) {
          if (!isPlaying || miss == 4) {
            showHighScoreTime = 1;
      ...
      if (showHighScoreTime > 0) { // isShowResetScreen
        showHighScoreTime = 0;
        resetGame();
        gameType = 1;
        startGame();
    ...
    isPressed = (digitalRead(button2Pin) == LOW) ? true : false;

    if (isPressed != isButton2Pressed) {
      ...
      if (isButton2Pressed) {
        if (buttonPressed == 0 && (add3Score == 0 || !isPlaying)) {
          ...
          if (!isPlaying || (isPlaying && (miss == 4 || (caught == 0 && ((clearMissCount > 0 && diverPosition > 0 && diverPosition <= 5) || (clearMissCount == 0 && diverPosition <= 5)))))) {
            if (!isPlaying || miss == 4) {
              showHighScoreTime = 1;
...
void resetGame() {
  ...
  for (int i = 0; i < 6; i++) {
    ...
    if (i < 2) {
      isRemainderVisibled[i] = true;
      isHourDigitVisibled[i] = false;
      isMinuteDigitVisibled[i] = false;
    }
